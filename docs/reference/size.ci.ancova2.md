...
plausible range for a conservatively large sample size. Note that the
within-group error variance is equal to the within-group variance times
1 minus the the squared correlation (s = 1) or squared multiple
correlation (s \> 1) between the response variable and the covariate(s).
